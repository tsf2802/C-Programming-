...
unsigned int hash(char *key) 
{
	...
	hashvalue = added % 7;
	return hashvalue; //Replace this.  Allows code to compile for now
}
...
struct binEntry *lookup(char *key){
	...
	unsigned int calculated = hash(key); //getting hash value for the certain given key
	//loops here in order to traverse throught the hashmap
	//check previously for any 
	
	if(hashtable[calculated] == NULL ) {
		return NULL;
	}
	bool bad = false;
	entry = hashtable[calculated];
...
void drop(char *key)
{
//	struct binEntry *remove_entry = lookup(key);
	unsigned int hashedkey = hash(key);
//	printf("printing value for lookup: %s \n", remove_entry->value);
